...
def transcribe_audio(file_path: str, max_retries: int = 3) -> str:
    """
    Transcribe an audio file via Together AI Whisper endpoint.
    Retries on 503 up to `max_retries`.
    """
    headers = {"Authorization": f"Bearer {TOG_TOKEN}"}
    files = {"file": open(file_path, "rb")}
    data = {
        "model": "openai/whisper-large-v3",  # free under Together's $1 credit tier
        "language": "en"
    }
    ...
    for attempt in range(1, max_retries + 1):
        ...
        # Together returns JSON with a "text" field
